...
# --- Rofi Communication Files ---
CMD_FILE = "/tmp/music_tui_cmd"          # Incoming commands
ROFI_LIST_FILE = "/tmp/music_tui_list"   # Results for Rofi
STATUS_FILE = "/tmp/music_tui_status"    # Status signal
...
class MusicTUI:
    ...
    def safe_addstr(self, y, x, text, attr=0):
        ...
        try: self.stdscr.addstr(y, x, text, attr)
        except: pass
    ...
    def clean_artist_name(self, artist_raw):
        ...
        for sep in separators:
            if sep in cleaned: cleaned = cleaned.split(sep)[0]
    ...
    def fetch_youtube_mix(self, video_id):
        try:
            ...
            cmd = ["yt-dlp", "--flat-playlist", "--playlist-end", "25", "--print", "%(id)s:::%(title)s:::%(uploader)s", mix_url]
            ...
            for line in output.splitlines():
                ...
                if vid_id not in seen_ids:
                    seen_ids.add(vid_id)
                    new_results.append({'track': title, 'artist': uploader, 'video_id': vid_id})
            return new_results
        except Exception: return []

    def get_youtube_url(self, query):
        try:
            cmd = ["yt-dlp", f"ytsearch1:{query} audio", "--print", "webpage_url", "--no-playlist", "--ignore-errors", "--no-warnings"]
    ...
    def draw_screen(self):
        ...
        if max_rows > 0:
            ...
            for i, idx in enumerate(range(start, end)):
                ...
                if 'video_id' in item: line = f"{item['track']}" 
                else: line = f"{item['track']} - {item['artist']}"
    ...
    def custom_input(self):
        ...
        while True:
            ...
            elif key == 27: curses.curs_set(0); return None
    ...
    def perform_search(self):
        ...
        if not query: self.draw_screen(); return
    ...
    def play_selection(self, song_item=None):
        if song_item: selected = song_item
        else:
    ...
    def start_smart_radio(self, prev_song):
        ...
        def _bg_smart():
            ...
            self.loading = False
            if mix_found: self.play_selection()
            else: self.status_message = "Could not generate radio."
    ...
    def handle_smart_navigation(self, explicit_prev_song=None):
        ...
        if self.mode == "SEARCH": self.start_smart_radio(prev_song)
        elif self.mode == "RADIO":
            if self.selected_index < len(self.results) - 1:
                self.selected_index += 1
                self.play_selection()
            else: self.start_smart_radio(prev_song)
   ...
   # --- UPDATED: External Command Listener in backup.py ---
    ...
    def run(self):
        ...
        while True:
            ...
            if key == ord('q'): self.player.stop(); break
                ...
                self.player.stop()
                self.perform_search()
            elif key in [10, 13, curses.KEY_ENTER]: self.play_selection()
            elif key == ord(' '): self.player.toggle_pause()
            elif key == ord('n'): self.skip_next()
            elif key == ord('b'): self.skip_prev()
            elif key == ord('x'): self.player.stop()
            elif key == ord('a'): self.autoplay = not self.autoplay

def main():
    if not shutil.which("yt-dlp"): print("Error: yt-dlp is required."); return
    if not shutil.which("mpv"): print("Error: mpv is required."); return
    try: curses.wrapper(MusicTUI)
